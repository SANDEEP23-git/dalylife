...
const OptionWheel = ({
  ...
  defaultSelected = 2,
  onChange,
  textColor = 'var(--text-muted, #888888)',
  activeColor = 'var(--accent, #4297ff)',
  side = 'left',
  fontSize = 1.8,
  spacing = 1.4,
  curve = 1,
  ...
  return (
    <div
      ...
      className={`option-wheel-root ${isDragging ? 'dragging' : ''} ${className}`}
      style={{
        position: 'relative',
        height: '100%',
        width: '100%',
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`option-wheel-item ${selectedIndex === index ? 'selected' : ''}`}
          style={{
            position: 'absolute',
            top: '50%',
            cursor: 'pointer',
            ...
            fontWeight: selectedIndex === index ? 650 : 300,
            letterSpacing: '-0.02em',
            transition: 'color 0.2s ease',
            color: `color-mix(in srgb, var(--ow-active-color) calc(var(--ow-p, 0) * 100%), var(--ow-text-color))`,
            ...(side === 'right'
              ? { right: 'var(--ow-inset)', transformOrigin: 'right center' }
              : { left: 'var(--ow-inset)', transformOrigin: 'left center' })
          }}
